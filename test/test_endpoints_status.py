def test_create_endpoint_status(client, user_cred):
    ...
    response = client.post("api/v1/endpoints/", json="login", headers={"Authorization": f"{user_cred.token_type} {user_cred.token}"})
    assert response.status_code == 200
...
def test_endpoint_not_found(client, user_cred):
    response = client.put("api/v1/endpoints/", json="get_users", headers={"Authorization": f"{user_cred.token_type} {user_cred.token}"})
