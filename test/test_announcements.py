...
def test_delete_announcement(client, test_user, test_announcements):
    ...
    token = login_res.json()["token"]
    res = client.delete(
        f"/api/v1/announcements/{test_announcements[0].id}", headers={'Authorization': f'Bearer {token}'})
...
def test_delete_announcement_does_not_exist(client, test_user):
    login_res = client.post(
        "/api/v1/users/login", data={"username": test_user["email"], "password": test_user["password"]})
    token = login_res.json()["token"]
    res = client.delete("/api/v1/announcements/1000", headers={'Authorization': f'Bearer {token}'})
...
def test_update_announcements(client, test_user, test_announcements):
    ...
    res = client.put("/api/v1/announcements/1", json=data, headers={'Authorization': f'Bearer {token}'})
...
def test_unauthorized_update_announcements(client):
    data = {"title": "update title", "content": "update content"}
    res = client.put("/api/v1/announcements/1", json=data)
...
def test_update_announcements_does_not_exist(client, test_user):
    ...
    res = client.put("/api/v1/announcements/100", json=data, headers={'Authorization': f'Bearer {token}'})
