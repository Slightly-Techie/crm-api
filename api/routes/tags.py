...
@tag_route.get("/tags")
def get_current_user_tags(
    user=Depends(get_current_user), db: Session = Depends(get_db)
):
    db_query = (
        db.query(User).filter(User.id == user.id).options(joinedload(User.tags)).first()
    )
...
def delete_tag_by_id(
    tag_id: int, user=Depends(get_current_user), db: Session = Depends(get_db)
):
